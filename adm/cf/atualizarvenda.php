...
$stmt = $pdo->prepare('SELECT * FROM registro_vendas WHERE id = ?');
$stmt->execute([$id]);
...
$nomev = $appointment['nomev'];
$numfiscal = $appointment['numfiscal'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $pdo->prepare('UPDATE registro_vendas SET nomev = ?, numfiscal = ?, data = ?, pagamento = ?, valortotal = ? WHERE id = ?');
    $stmt->execute([$nomev, $numfiscal, $data, $pagamento, $valortotal, $id]);
    header('Location: listavendas.php');
    exit;
}
?>
    ...
    <script type="text/javascript">
        function confirmDelete(event, id) {
            ...
            confirmBtn.onclick = function() {
                window.location.href = '../../app/controllers/deletarvenda.php?id=' + id;
            }
        }
...
<header>
        <div class="esq">
            ...
            <div class="rh-ico">
                <a href="novoproduto.php"><img src="../../public/resources/assets/img/cf.png"></a>
                <h1>CF</h1>
    ...
    <section class="content">
        ...
        <div class="main-title">
            <div class="title-section">
                ...
                <h2>Este formulario permite que você atualize uma venda</h2>
            </div>
...
<label for="nomev">Nome:</label>
    <input type="text" name="nomev" value="<?php echo $nomev; ?>" required></br>

    <div class="dd-content">
    <label for="numfiscal">Número Fiscal:</label>
    <input type="number" name="numfiscal" value="<?php echo $numfiscal; ?>" required></br>
    </div>
    </div>
    <div class="baixo">
    <div class="dire">
    ...
    <input type="number" name="valortotal" value="<?php echo $valortotal; ?>" required></br>
    </div>
    </div>
    </div>
